var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      // S.UI.simulate('F2EMAN|Shifter|Type|to start|#rectangle|#countdown 3||');
      S.UI.simulate('#time||F2EMAN');
    }
...
S.UI = (function () {
  var input = document.querySelector('.tool-input'),
  ...
  function checkInputWidth(e) {
    if (input.value.length > 18) {
      tool.classList.add('tool--wide');
    } else {
      tool.classList.remove('tool--wide');
    }

    if (firstAction && input.value.length > 0) {
      tool.classList.add('tool--enter');
    } else {
      tool.classList.remove('tool--enter');
    }
  }
